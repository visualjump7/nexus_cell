...
export function BriefBlockEditor({
  block,
  liveData,
  onUpdate,
}: BriefBlockEditorProps) {
  ...
  if (block.type === "bills") {
    const daysAhead =
      (block.config?.days_ahead as number | undefined) ?? 7;
  ...
  if (block.type === "projects") {
    const status =
      (block.config?.status as string | undefined) ?? "all";
